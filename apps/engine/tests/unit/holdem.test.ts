...
describe('Texas Hold\'em', () => {
  describe('dealHand - Hold\'em mode', () => {
    ...
    it('should post blinds in heads-up', () => {
      ...
      const sbPlayer = result.updatedPlayers.find(p => p.seat_number === result.gameState.button_seat);
      const bbPlayer = result.updatedPlayers.find(p => p.seat_number !== result.gameState.button_seat);
    ...
    it('should set action to UTG in multi-way preflop', () => {
      ...
      expect((result.gameState.seats_to_act as number[]).length).toBeGreaterThan(0);
    });
  ...
  describe('Hold\'em street transitions', () => {
    ...
    it('should maintain action order on postflop streets', () => {
      ...
      const ctx2: ActionContext = {
        ...ctx,
        gameState: { ...gameState, ...result1.updatedGameState } as GameStateRow,
