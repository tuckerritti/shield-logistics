...
describe('Phase Transitions', () => {
  describe('Flop to Turn', () => {
    it('should advance to turn when all players check on flop', () => {
      ...
      expect(result.updatedGameState.board_state?.board1).toEqual(['Ah', 'Kh', 'Qh', 'Jh']);
      expect(result.updatedGameState.board_state?.board2).toEqual(['2c', '3c', '4c', '5c']);
    });
    ...
    it('should reset bets when advancing to turn', () => {
      ...
      const updatedPlayer1 = result.updatedPlayers.find(p => p.seat_number === 1);
      const updatedPlayer2 = result.updatedPlayers.find(p => p.seat_number === 2);
  ...
  describe('Turn to River', () => {
    it('should advance to river when betting round completes', () => {
      ...
      expect(result.updatedGameState.board_state?.board1).toEqual(['Ah', 'Kh', 'Qh', 'Jh', 'Th']);
      expect(result.updatedGameState.board_state?.board2).toEqual(['2c', '3c', '4c', '5c', '6c']);
    });
  });
  ...
  describe('River to Showdown', () => {
    ...
    it('should calculate side pots at showdown', () => {
      const players = [
        createPlayer({ seat_number: 1, current_bet: 0, chip_stack: 0, total_invested_this_hand: 50, is_all_in: true }),
        createPlayer({ seat_number: 2, current_bet: 0, chip_stack: 450, total_invested_this_hand: 50 }),
        createPlayer({ seat_number: 3, current_bet: 0, chip_stack: 300, total_invested_this_hand: 200 })
      ];
  ...
  describe('All-in scenarios ending hand early', () => {
    it('should advance directly to showdown when only all-in players remain', () => {
      const players = [
        createPlayer({ seat_number: 1, current_bet: 50, chip_stack: 0, is_all_in: true }),
        createPlayer({ seat_number: 2, current_bet: 50, chip_stack: 0, is_all_in: true }),
    ...
    it('should complete hand immediately when everyone folds to last player', () => {
      const players = [
        createPlayer({ seat_number: 1, current_bet: 0, chip_stack: 500, has_folded: true }),
        createPlayer({ seat_number: 2, current_bet: 0, chip_stack: 500, has_folded: false }),
        createPlayer({ seat_number: 3, current_bet: 10, chip_stack: 490 })
      ];
  ...
  describe('Multi-street progression', () => {
    it('should skip all-in players from new street action', () => {
      const players = [
        createPlayer({ seat_number: 1, current_bet: 100, chip_stack: 0, is_all_in: true }),
